-- Um atendente não pode ter dois atendimentos no mesmo dia e horário, com ou sem sala.
-- A trava por workspace/data/horário (pg_advisory_xact_lock) já serializa reservas
-- concorrentes do mesmo horário, então a checagem abaixo não sofre corrida.
...
as $$
declare target_workspace uuid; appointment public.agenda_items; appointment_date date; appointment_time time; uses_room boolean; room_name text; attendant text;
begin
  ...
  attendant := nullif(p_data ->> 'atendente', '');
  if attendant is null then raise exception 'Informe o atendente.'; end if;
  perform pg_advisory_xact_lock(hashtext(target_workspace::text || appointment_date::text || appointment_time::text));
  if exists (select 1 from public.agenda_items where workspace_id = target_workspace and kind = 'appointment' and status in ('active', 'realized') and data ->> 'data' = appointment_date::text and data ->> 'horario' = to_char(appointment_time, 'HH24:MI') and data ->> 'atendente' = attendant) then raise exception 'Este atendente já tem um atendimento neste dia e horário.'; end if;
...
revoke execute on function public.schedule_agenda_appointment(uuid, jsonb) from public, anon;
grant execute on function public.schedule_agenda_appointment(uuid, jsonb) to authenticated;
